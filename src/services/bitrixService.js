 ...
 */
export async function getAllDealCategories(b24Service) {
    try {
        const response = await b24Service.call('crm.category.list', {
            entityTypeId: 2
        });
        ...
        console.error('Ошибка getAllDealTypes:', error);
...
/**
 * Получение списка полей сделки определенной воронки
 */
export async function getFieldsDeal(b24Service) {
    try {
        const response = await b24Service.call('crm.deal.fields', {});
        
        const result = response.result || response;
        console.log('Полученные поля: ', result);
        if (result) {
